RSpec.describe 'inject pattern test' do
  ...
  it 'test 5' do
    ...
    airlines.each do |airline|
      letters = airline.length
      number_of_letters[airline] = letters
    end
  ...
  it 'test 8' do
    ...
    total_calories = 0
    toppings.each do |(item, values)|
      total_calories += (values[:calories] * values[:quantity])
    end
  ...
  it 'test 9' do
    ...
    grades.each do |category, scores|
      final_grade += scores.sum(0.0) / scores.length
    end
  ...
  it 'test 10' do
    ...
    printable_menu = "Menu:\n"
    menu.each do |item, prop|
      item_line = "- "
      prop[:flavors].each do |flavor|
          if prop[:flavors].last == flavor
              item_line << "and #{flavor} #{item}"
          else
          item_line << "#{flavor}, "
          end
      end
      if prop[:gluten_free]
          item_line << " (gluten free)\n"
      else
          item_line << " (non gluten free)\n"
      end
      printable_menu << item_line
  end
    expected =  "Menu:\n"\
