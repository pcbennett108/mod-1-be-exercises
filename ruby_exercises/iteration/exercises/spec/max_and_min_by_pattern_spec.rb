RSpec.describe 'max and min by pattern' do
  ...
  it 'test 5' do
    ...
    most_digits = stats[0]
    stats.each do |stat|
      if most_digits.to_s.length < stat.to_s.length
        most_digits = stat
      end
    end
  ...
  it 'test 9' do
    ...
    programmers.each do |group|
      if fewest_programmers.length > group.length
  ...
  it 'test 10' do
    ...
    fewest_programmers_num = programmers.first[1].length
    fewest_programmers = programmers.first[0]
    programmers.each do |lang, group|
      if group.length < fewest_programmers_num
          fewest_programmers_num = group.length
